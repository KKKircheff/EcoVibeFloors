...
import {User, Product, Order, ConsultationRequest, COLLECTIONS} from './types';
...
export const getDocRef = {
    user: (id: string): DocumentReference<User> => doc(collections.users, id),

    product: (id: string): DocumentReference<Product> => doc(collections.products, id),

    order: (id: string): DocumentReference<Order> => doc(collections.orders, id),

    consultation: (id: string): DocumentReference<ConsultationRequest> => doc(collections.consultations, id),

    review: (id: string): DocumentReference => doc(collections.reviews, id),

    category: (id: string): DocumentReference => doc(collections.categories, id),

    supplier: (id: string): DocumentReference => doc(collections.suppliers, id),
};
...
export const collectionUtils = {
    ...
    async checkCollectionHealth(collectionName: string): Promise<{
        ...
        try {
            const _collectionRef = collection(db, collectionName);
    ...
    async exportCollection(collectionName: string): Promise<Record<string, unknown>[]> {
    ...
    async importCollection(collectionName: string, data: Record<string, unknown>[]): Promise<void> {
