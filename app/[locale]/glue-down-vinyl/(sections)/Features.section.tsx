...
export async function Features() {
    const t = await getTranslations('whatIsGlueDownVinyl.features');

    const features = [
        { key: 'waterproof' },
        { key: 'permanentInstall' },
        { key: 'warranty' },
        { key: 'ecoFriendly' },
        { key: 'commercialGrade' },
        { key: 'soundDampening' }
                        ...
                        <Grid key={feature.key} size={{ xs: 12, md: 6 }}>
                            <Stack spacing={1}>
                                <BlogSubtitle color={'primary.50'}>
                                    {t(`${feature.key}.title` as keyof IntlMessages['whatIsGlueDownVinyl']['features'])}
                                </BlogSubtitle>
                                <BlogContent color={'primary.100'}>
                                    {t(`${feature.key}.description` as keyof IntlMessages['whatIsGlueDownVinyl']['features'])}
                                </BlogContent>
                            </Stack>
                        </Grid>
                    ))}
